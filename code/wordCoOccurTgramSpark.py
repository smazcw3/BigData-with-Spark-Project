...
def lemma_combinatorics(lemma_list1, lemma_list2, lemma_list3):
    result = []
    for i in lemma_list1:
	for j in lemma_list2:
            for k in lemma_list3:
	        result.append((i,j,k)) #returning tuple
    return result
...
def test_map(tokenized):
    ...
    if convert_tokenized.startswith("<luc.") or convert_tokenized.startswith("<verg."):
        ...
        for j in range(0, len(combined_list)):
            ...
            normalized_word3 = normalize(combined_list[j][2])
            lemmas_for_word1 = getLemmasList(normalized_word1)
            lemmas_for_word2 = getLemmasList(normalized_word2)
            lemmas_for_word3 = getLemmasList(normalized_word3)
            lemma_combinations_list = lemma_combinatorics(lemmas_for_word1, lemmas_for_word2, lemmas_for_word3)        
...
if __name__ == "__main__":
    ...
    reducer_output.saveAsTextFile("spark_trigram_output")
